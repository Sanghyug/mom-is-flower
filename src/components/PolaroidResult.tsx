...
export default function PolaroidResult({
  ...
  const handleShare = async () => {
    try {
      ...
      // Base64 이미지를 실제 이미지 파일로 변환
      const response = await fetch(finalImageBase64);
      const blob = await response.blob();
      const file = new File([blob], `${flowerName}_card.png`, {
        type: "image/png",
      });

      // 스마트폰에서 이미지 파일 공유가 가능한 경우
